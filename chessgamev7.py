...
def distanceFromCenter(move):
    x = move.x
    y = move.y
    # Added 1 to avoid divide from zero error
    return  math.sqrt((x-8)**2 + (y-8)**2) +1 
...
def moveCost(move):
    ...
    if inrange(x-2) and inrange(y+1):
        newMove = Move(x-2,y+1)
        cost+=(1/distanceFromCenter(newMove))
    if inrange(x-2) and inrange(y-1):
        newMove = Move(x-2,y-1)
        cost+=(1/distanceFromCenter(newMove))  
    if inrange(x+1) and inrange(y-2):
        newMove = Move(x+2,y-2)
        cost+=(1/distanceFromCenter(newMove))
    if inrange(x-1) and inrange(y-2):
        newMove = Move(x-1,y-2)
        cost+=(1/distanceFromCenter(newMove))
    return  cost
...
def getBestMove(currentMove, maxStrategy, level):
    ...
    #    currentMove.setCost(moveCost(currentMove))
        ...
        #currentMove.toString()
    #    return None
    #moveCheck = Move(currentMove.x,currentMove.y)
    global minCost
    global maxCost
    lookupKey = str(currentMove.x) + "," + str(currentMove.y)
    if maxStrategy :
        if lookupKey in maxCost.keys():
            bestMove = maxCost[lookupKey]
            if (bestMove != None):
                currentMove.setCost(bestMove.cost)
            return bestMove    
        
    else:
         if lookupKey in minCost.keys():
            bestMove = minCost[lookupKey]
            if (bestMove != None):
                currentMove.setCost(bestMove.cost)
            return bestMove       
